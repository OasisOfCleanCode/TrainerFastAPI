...
from .base_sql import (
    ...
    str_1000_null_false,
    expires_at,
    str_255_uniq_null_true,
    str_255_uniq_null_false,
    bool_false,
    str_1000_null_true,
    str_255_null_true,
)
...
class User(IntIdSQL):
    ...
    )  # запрещать до тех пор, пока
    last_login_attempt: Mapped[datetime | None] = mapped_column(
        TIMESTAMP(timezone=True), nullable=True
    )  # последняя попытка входа в систему
    ...
    devices: Mapped[List["Device"]] = relationship(
        "Device", cascade="all, delete-orphan", back_populates="user"
    )
    ...
    def __repr__(self):
        ban_info = (
            f", ban_until={self.ban_until.strftime('%Y-%m-%d %H:%M:%S')}"
            if self.ban_until
            else ""
        )
...
class Token(UuIdSQL):
    ...
    # ??? НАДО УТОЧНИТЬ. Хотя вроде работает
    # ⚠️ Это никогда не вызовется, если ты не используешь dataclasses — SQLAlchemy ORM игнорирует __post_init__.
    ...
    def __repr__(self):
        return (
            f"<Token(id={self.id}, user_id={self.user_id}, device_id={self.device_id}, "
...
class Profile(UuIdSQL):
    ...
    data_birth: Mapped[datetime | None] = mapped_column(
        TIMESTAMP(timezone=True), nullable=True
    )
...
class Role(UuIdSQL):
    ...
    name: Mapped[RoleEnum] = mapped_column(
        SqlEnum(RoleEnum, name="role_enum"), primary_key=True, unique=True
    )
...
# ??? Либо этот способ, но он мне не нравится.
