...
REFRESH_RATE_LIMIT = "10/minute"  # 10 запросов в минуту
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if not request.url.path.endswith(
            ...
            logger.bind(log_type="route_log").debug(
                f"Запрос обработан: user_id={user_id}, метод={request.method},"
                f" адрес={request.url.path}, статус={response.status_code}, время={process_time:.2f}s"
            )
    ...
    async def processing_requests_for_other_methods(
        ...
        try:
            ...
            if (
                ...
                or origin is None
            ):  # Для запросов из Postman/Insomnia (без Origin header)
...
class StaticVersionMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Если это шаблон — подменяем context
